...
export default function NewMed(Modal: Readonly<NewMedProps>) {
  const [newName, setNewName] = useState('');
  const [pills, setPills] = useState(0);
  const [selectedTime, setSelectedTime] = useState('');
  const [times, setTimes] = useState([] as string[]);
  function submit() {
    Modal.confirm({ name: newName, pills: pills, takeAt: times });
  }
  ...
  return (
    <div className='fixed p-5 flex flex-col top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 md:top-0 md:left-0 md:translate-x-1/2 md:translate-y-1/2 z-10 min-w-80 md:min-w-1/2 min-h-1/2 overflow-hidden bg-gray-300 text-black border-3 border-gray-700 rounded-xl'>
      <h1 className='text-black w-full bg-amber-300'>Add New Med</h1>
      <div className='flex flex-col gap-2.5 p-2 '>
        {/* <div className='flex flex-col gap-2.5 p-2 bottom-0 md:bottom-10 left-1/2 -translate-y-5 -translate-x-1/2'> */}
        <form action={submit}>
          <div>
          ...
          <div>
            ...
            <input
              ...
              value={pills}
              onInput={e => {
                setPills(Number.parseInt(e.currentTarget.value));
              }}
            />
          </div>
          <div>
